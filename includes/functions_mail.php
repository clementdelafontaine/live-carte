...
//Create a new PHPMailer instance
	...
	$mail->Subject =  $sujet/*"test sujet mail"*/;
	...
	//if ($corps[0]["webService_Atlas"]=='oui'){//verification que la course soit bien enregistré Atlas
		/*if (isset($corps[0].["groupe"])){
			$IDAtlas=extract_champ_epreuve('IDAtlas',$id_epreuve);//on récupere le raceID de l'epreuve
			
			$data = array(
				"raceID"=>$corps[0]["IDAtlas"],"name"=>$_POST["nomGroupe"],"creationRequestedBy"=>"66763D92-630C-4C0A-AB6F-7E9403F791D8"
...
//echo "reponse:".$reponse;
	...
	//enregistrement de l'internaute dans notre bd atlas
		$query_b = "INSERT INTO `k_Atlas`( `IDAtlas`, `idInternaute`, `idInscriptionInternaute`, `IDGroupeAtlas`, `nomGroupe`) ";
		$query_b .= "VALUES (".$corps[0]["IDAtlas"].",".$corps[0]["idInternaute"].",".$corps[0]["idInscriptionEpreuveInternaute"].",null,null)')";
		$result_b = $mysqli->query($query_b);
		/*
	// envoie de la requete a atlas 
		if ($corps["sexeInternaute"]="homme"){//on inscrit le sexe en tant que 0 ou 1 car c'est de ce type la qu'il le souhaite(Atlas)
		$sexe=0;
	}elseif($corps["sexeInternaute"]="femme"){
		$sexe=1;
	}

	$data = array(
		"crmid"=>null,"raceID"=>$corps[0]["IDAtlas"],"deviceID"=>null,"email"=>$corps[0]["emailInternaute"],"externalEntryID"=>null,"firstName"=>$corps[0]["prenomInternaute"],"lastName"=>$corps[0]["nomInternaute"],"bib"=>null,"podSerial"=>null,"dob"=>$corps[0]["birthdate"],"age"=>null,"gender"=>$sexe,"categoryExternalReg"=>null,"categoryID"=>null,"teamID"=>null,"creationRequestedBy"=>"66763D92-630C-4C0A-AB6F-7E9403F791D8"
...
if ($donnees[0]['valeur_code_promo'] > 0) { 
...
function send_mail_contact( $data )
{
	...
	$mail->AddAddress( "alicia@ats-sport.com" );
...
function send_mail_admin_new_event( $data )
{
	global $mysqli;
	//Type de mail et méthode d'envoi
	$mail = new PHPmailer();
	...
	$mail->Subject = "[ATS-SPORT.COM] : Urgent demande Appli inscription";
...
function ouverture_inscriptions($data)
{
	...
	if( !$data['reglement'] )
		$body .='<p style="font: 15px/1.25em \'Helvetica Neue\',Arial, Helvetica; color: #333;">Nous n\'avons pas reçu le <u>réglement de l\'épreuve</u>, merci de l\'envoyer à alicia@ats-sport.com dès que possible.</p><br>';
