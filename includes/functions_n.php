...
function ajoutTeamAtlas($corps) {
	$data = array(
				"raceID"=>$corps[0]["IDAtlas"],"name"=>$corps[0]["groupe"],"creationRequestedBy"=>"66763D92-630C-4C0A-AB6F-7E9403F791D8"
...
function ajoutUserAtlas($corps) {
if ($corps[0]["sexeInternaute"]=='M'){//on inscrit le sexe en tant que 0 ou 1 car c'est de ce type la qu'il le souhaite(Atlas)
		$sexe=0;
		}elseif($corps[0]["sexeInternaute"]=='F'){
		$sexe=1;
		}
		$email="".$corps[0]['idInscriptionEpreuveInternaute']."@ats.com";

	$data = array(
		"crmid"=>null,"raceID"=>$corps[0]["IDAtlas"],"deviceID"=>null,"email"=>$email,"externalEntryID"=>null,"firstName"=>$corps[0]["prenomInternaute"],"lastName"=>$corps[0]["nomInternaute"],"bib"=>null,"podSerial"=>null,"dob"=>$corps[0]["naissanceInternaute"],"age"=>18,"gender"=>$sexe,"categoryExternalReg"=>null,"categoryID"=>null,"teamID"=>$corps[0]["teamID"],"creationRequestedBy"=>"66763D92-630C-4C0A-AB6F-7E9403F791D8"

	);
	
		$data_string=json_encode($data);// on encode en JSON car ATLAS ne recoit que des requetes utilisant du JSON
		$ch=curl_init();
		curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "POST");

		curl_setopt($ch, CURLOPT_URL, "https://public.sandbox.atlaslivetracking.com/api/Entry");
		curl_setopt($ch, CURLOPT_POSTFIELDS, $data_string);
		curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, true);
		curl_setopt($ch, CURLOPT_POST, true);
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
...
function extract_options_plus($champ, $idOptionsPlus)	
{
global $mysqli;	
		$query  = "SELECT ".$champ;
		$query .= " FROM r_options_plus ";
		$query .=" WHERE idOptionPlus = ".$idOptionsPlus;
		$result = $mysqli->query($query);
		$row=mysqli_fetch_row($result);
...
global $mysqli;	
	$query_info   = "SELECT DISTINCT riit.idInternaute, riit.idInscriptionEpreuveInternaute, riit.idEpreuve,riit.idEpreuveParcours, ";
	//$query_info   = "riit.idInternauteReferent, ";
	$query_info   .= "riit.idEpreuveParcoursTarif,ri.nomInternaute,ri.prenomInternaute,ri.emailInternaute,ri.dateInscription,riit.id_session, riit.montant_inscription,riit.participation,riit.idEpreuveParcoursTarifPromo, riit.label_code_promo,riit.montant_code_promo,riit.place_promo, riit.valeur_place_promo,riit.info_diverses,riit.equipe,riit.groupe, riit.categorie, riit.idOptionPlus, riit.Prix_OptionPlus,riit.assurance,";
	...
	$query_info   .= "rept.tarif ";
	//$query_info   .= ",ricsd.idChampsSupDotation, ricsd.value ";
	$query_info .= "FROM r_inscriptionepreuveinternaute as riit ";
	...
	$query_info .= " INNER JOIN r_epreuveparcours as rep ON riit.idEpreuveParcours = rep.idEpreuveParcours ";
	$query_info .= " INNER JOIN r_epreuveparcourstarif as rept ON riit.idEpreuveParcoursTarif = rept.idEpreuveParcoursTarif ";
	...
	$query_info .= "WHERE riit.idInscriptionEpreuveInternaute = ".$id_internaute." AND riit.paiement_type in('ATTENTE','CB','ATTENTE CHQ')";
...
global $mysqli;	
	...
	$query_info   .= "rii.idEpreuveParcoursTarif,ri.nomInternaute,ri.prenomInternaute,ri.emailInternaute,rii.date_insc,ri.passInternaute, rii.verif_certif as certificatMedical, rii.verif_auto_parentale as autoParentale, rii.id_session as idSession,rii.paiement_montant,(rii.montant_inscription-rii.participation-rii.Prix_OptionPlus) as cout,rii.frais_cb,rii.participation, rii.frais_cheque,rii.info_cheque,rii.idEpreuveParcoursTarifPromo, rii.label_code_promo as code_promo,rii.montant_code_promo as valeur_code_promo,rii.place_promo, rii.valeur_place_promo, rii.info_diverses,rii.equipe, rii.groupe,rii.categorie, rii.assurance, rii.idOptionPlus, rii.Prix_OptionPlus, rii.InscritParidInternaute,  ";
	$query_info   .= "re.nomEpreuve, re.dateEpreuve,re.webService_Atlas ,re.IDAtlas, rep.nomParcours, rep.horaireDepart, rep.idEpreuveParcours, rep.relais, ";
	$query_info   .= "ri.sexeInternaute, ri.naissanceInternaute, ri.clubInternaute, ri.adresseInternaute, ri.cpInternaute, ri.villeInternaute, ri.paysInternaute, ";
	$query_info   .= "rept.idEpreuveParcoursTarif, rept.tarif ";
	//$query_info   .= ",ricsd.idChampsSupDotation, ricsd.value ";
	...
	$query_info .= " INNER JOIN r_epreuveparcourstarif as rept ON rii.idEpreuveParcoursTarif = rept.idEpreuveParcoursTarif ";
	//$query_info .= " INNER JOIN r_insc_champssupdotation as ricsd ON (riit.idInternaute = ricsd.idInternaute ";
	//$query_info .= " AND riit.idEpreuve = ricsd.idEpreuve AND riit.idParcours = ricsd.idEpreuveParcours) ";
...
global $mysqli;	
	...
	if ($update_internaute == TRUE ) {
		...
		//$label = extract_champ_code_promo('label',$idEpreuveParcoursTarifPromo;
		$query ="UPDATE r_inscriptionepreuveinternaute SET ";
...
function check_internaute_existant_v3 ($nom,$prenom,$date_naissance,$sexe) {
global $mysqli;
	//naissanceInternaute
	//sexeInternaute
		$query  = "SELECT idInternaute FROM r_internaute as ri"; 
		...
		$query .= " AND ri.dateConsultation != 0 ";
		/*
		if(!empty($idInternaute)) {
			$query .= " AND ri.idInternaute = ".$idInternaute." ";
		}
		*/
		$result = $mysqli->query($query);
...
curl_setopt_array($curl, array(
    ...
    CURLOPT_URL => 'https://webservicesffa.athle.fr/st_chrono/stchrono.asmx/STCHRONO?UID=YASC&MDP=Atsats2!&NUMREL='.$num_lic.'&NOM='.$nom.'&PRENOM='.$prenom.'&SEXE='.$sexe.'&DATENAI='.$date_n.'&CMPCOD='.$CMPCOD.'&ID_ACT_EXT=&ID_CMP_EXT=&MSG_RETOUR=HTTP/1.1'
...
function evenements_a_venir( $nb = 5 )
{
	...
	$qevenement .= "AND paiement_cb like '1' AND idEpreuve != 4247 ";
