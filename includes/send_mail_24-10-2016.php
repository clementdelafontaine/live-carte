...
if ($nb_inscrit > 1 ) {
	...
	$query = "SELECT ri.idInternauteref, rii.idEpreuveParcours, riei.idInscriptionEpreuveInternaute FROM r_internautereferent as ri ";
...
while (($row=mysqli_fetch_array($result)) != FALSE) {
	...
	if ($type_paiement== 'GRATUIT') {
		...
		if ($nb_inscrit > 1 && $row['idInscriptionEpreuveInternaute'] != $id_internauteInscription_ref ) {
			...
			$sujet = "PointCourse - Inscription à l'epreuve ".$row_info['nomEpreuve']." - ".$row_info['nomParcours'];
			$mail_send = send_mail ('inscriptions@pointcourse.com','Point Course Inscription',$row_info['emailInternaute'], $row_info['nomInternaute']." ".$row_info['prenomInternaute'], $sujet,$donnees);			
		   $dossard = numerotation($row['idParcours'],$row['idEpreuve'],$row['idInscriptionEpreuveInternaute']);
			...
			if (!empty($email_organisateur)) {
				...
				$donnees[0]=$row_info;
				$mail_send = send_mail ('inscriptions@pointcourse.com','Point Course Inscription',$email_organisateur, $row_info['nomInternaute']." ".$row_info['prenomInternaute'], $sujet,$donnees);
			}
		}
		elseif ($nb_inscrit > 1 && $row['idInscriptionEpreuveInternaute'] == $id_internauteInscription_ref ) 
		{
			...
			$sujet = "PointCourse - Inscription à l'épreuve ".$row_info['nomEpreuve']." - ".$row_info['nomParcours'];
			$mail_send = send_mail ('inscriptions@pointcourse.com','Point Course Inscription',$row_info['emailInternaute'], $row_info['nomInternaute']." ".$row_info['prenomInternaute'], $sujet,$donnees);			
			$dossard = numerotation($row['idParcours'],$row['idEpreuve'],$row['idInscriptionEpreuveInternaute']);
	...
	$sujet = "PointCourse - Inscription Epreuve ".extract_champ_epreuve('nomEpreuve', $id_epreuve)." - ".extract_champ_parcours('nomParcours',$row['id_parcours']);
	...
	$mail_send = send_mail ('contact@pointcourse.com','Point Course Inscription',$row['emailInternaute'], $row['nomInternaute']." ".$row['prenomInternaute'], $sujet,$corps,"jf@chauveau.nom.fr");
	if ($mail_send == 'ok') {
...
/*								
$sujet = "PointCourse - Inscription Epreuve ".extract_champ_epreuve('nomEpreuve', $id_epreuve)." - ".extract_champ_parcours('nomParcours',$id_parcours);
...
//****$mail_send = send_mail ('contact@pointcourse.com','Point Course Inscription',$value['email'], $value['nom']." ".$value['prenom'], $sujet,$corps,"jf@chauveau.nom.fr"); */
