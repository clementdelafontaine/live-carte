<?php 
	...
	connect_db();
	require_once('functions_cyfac.php');
	...
	if ($_GET['id_parcours'] !=0) {
		...
		$champ_participation = array ('id' => 'idChampsSupParticipation', 'nom' =>'nom', 'label'=>'label','type_champ'=>'type_champ','prix'=>'prix', 'qte'=>'qte','date_butoir'=>'date_butoir', 'obligatoire'=>'obligatoire', 'ordre'=>'ordre', 'information'=>'information');
		...
		while($row_questiondiverse = mysqli_fetch_array($result_questiondiverse))
		{
			...
			if ($json_all) { $json_all = $json_ages_limites.$separator_fonction.$json_tarif_et_promo.$separator_fonction.$json_option_plus.$separator_fonction.$json_all; } else {  $json_all = $json_ages_limites.$separator_fonction.$json_tarif_et_promo.$separator_fonction.$json_option_plus; };
