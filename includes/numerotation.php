...
function numerotation($parcours,$epreuve,$iei,$idInscriptionEpreuveInternaute_parent=0)
{
	...
	$insc_dossard_dernier = extract_champ_epreuve('insc_dossard_dernier',$epreuve);
	...
	{							
		...
		if($dossardmin == 0)
		{
			...
			if ( $insc_dossard_dernier == 'oui')
			{
				$query_max = "SELECT max(dossard) as doss FROM r_inscriptionepreuveinternaute WHERE idEpreuve = ".$epreuve." and idEpreuveParcours = ".$parcours."";
				$result_max = $mysqli->query($query_max);
				$dossard_max= mysqli_fetch_array($result_max);
				$doss = $dossard_max['doss']+1;
...
function numerotation_no_update($parcours,$epreuve)
{
	global $mysqli;
	$dossard_affecte = array();
	$insc_dossard_dernier = extract_champ_epreuve('insc_dossard_dernier',$epreuve);
