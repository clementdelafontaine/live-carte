...
Function nombre_dossard ($id_parcours) {
	...
	if ($dossardmin > 0 && $dossardmax > 0 ) {
		...
		$nb_dossard_disponible = ($nb_dossard_du_parcours - $nb_dossard_deja_attribue-$nb_exclusion);
	}
	else
	{
...
function inscription_perso($idEpreuve,$idEpreuveParcours='',$codeactivation='')	{
	...
	$query_ep="SELECT groupe, codeActivation, idEpreuveParcours from r_epreuveperso_pre WHERE idEpreuve = ".$idEpreuve;
