...
require_once("../etransaction/class.paiement.php");
	...
			$fp = fopen("logs_send_mail.txt","a");		fputs($fp,"SEND_MAIL : ".date("d/m/Y H-i-s")." : ".$_SERVER['REQUEST_URI']."\n");		fputs($fp," : --------------------------------------------------\n");		
...
$id_assurance_annulation = $_POST['id_assurance_annulation'];$assus = $_POST['assu'];//echo "assus - ".$assus."----";$assu_first = explode("|", $assus);//print_r($assu_first);$assure=array();foreach ($assu_first as $key=>$assu){	//echo "</br>".$assu."----".$key."</br>";    $assu_tab= explode("_", $assu);	//echo $assu_tab[3];	$assure[$key]['id']=$assu_tab[2];	$assure[$key]['montant']=$assu_tab[3];}	//print_r($assure);function in_array_r($needle, $haystack, $strict = false) {    foreach ($haystack as $item) {        if (($strict ? $item === $needle : $item == $needle) || (is_array($item) && in_array_r($needle, $item, $strict))) {           			return $item;        }    }    return false;}		
$tab_temp = array();
...
$query .= "AND rii.idSession = '".$id_session_unique."' ";$query .=" ORDER BY rii.idInscriptionEpreuveInternaute  DESC LIMIT ".$nb_inscrit;
$result = $mysqli->query($query);
...
while (($row=mysqli_fetch_array($result)) != FALSE) {
	...
	if ($type_paiement== 'CHQ') {
		...
		if ($nb_inscrit > 1 && $row['idInscriptionEpreuveInternaute'] != $id_internauteInscription_ref) {
			...
			$frais_cheque_coureur_total+=$frais_cheque_coureur;
			
			$q1  = "UPDATE r_inscriptionepreuveinternaute SET paiement_type ='ATTENTE CHQ', frais_cheque = ".$frais_cheque_coureur." ";
			$q1 .= "WHERE idInscriptionEpreuveInternaute = ".$row['idInscriptionEpreuveInternaute'];
			//echo "inscrit pas référent : ".$q1;
			$mysqli->query($q1);
			/*** LOGS ****/			fputs($fp," : CHQ : mail_send (non referent) : ".$q1."\n");			/*** LOGS ****/
			$donnees = array();
		...
		elseif ($nb_inscrit > 1 && $row['idInscriptionEpreuveInternaute'] == $id_internauteInscription_ref ) 
		{
			...
			$frais_cheque_coureur_total+=$frais_cheque_referent;
			$q1  = "UPDATE r_inscriptionepreuveinternaute SET paiement_type ='ATTENTE CHQ', frais_cheque = ".$frais_cheque_referent." ";
			$q1 .= "WHERE idInscriptionEpreuveInternaute = ".$row['idInscriptionEpreuveInternaute'];
			/*** LOGS ****/			fputs($fp," : CHQ : mail_send (referent) : ".$q1."\n");			/*** LOGS ****/
			$mysqli->query($q1);
		...
		/*** LOGS ****/
			if($payeur == 'coureur') $montant_referent = ($cout+$frais_cheque_referent); else $montant_referent = $cout;
			$queryupiei=" UPDATE r_insc_internautereferent
			SET paiement_type='ATTENTE CHQ', montant = ".($montant_referent).", frais_cb=".$frais_cheque_coureur_total." 
			WHERE idInternauteReferent = ".$id_referent;
			$resultupiei = $mysqli->query($queryupiei);
		...
		else
		{
			...
			$q1  = "UPDATE r_inscriptionepreuveinternaute SET paiement_type ='ATTENTE CHQ', frais_cheque = ".$cout_paiement_cheque." ";
			$q1 .= "WHERE idInscriptionEpreuveInternaute = ".$row['idInscriptionEpreuveInternaute'];
			/*** LOGS ****/			fputs($fp," : CHQ : mail_send (referent_enfant ?) : ".$q1."\n");			/*** LOGS ****/
			$mysqli->query($q1);
		...
		if ($mail_send == 'ok') {
			...
			SET  typePaiement='ATTENTE CHQ', frais_cheque = ".$frais_cheque."
			WHERE idEpreuve = ".$row['idEpreuve']."
	...
	*/
		...
		//echo "id : ".$row['idInscriptionEpreuveInternaute']."###";		$search_assure = in_array_r($row['idInscriptionEpreuveInternaute'], $assure);		//print_r($search_assure);		$frais_assurance = 0;		if ($relais !='oui') $relais='non';
		if ($search_assure==true)		{			//echo "JASSURE  !!! ".$assus."----".$assu."-----".$row['idInscriptionEpreuveInternaute'];								$bdd_assu  = "INSERT INTO r_insc_assurance_annulation (idEpreuve, idEpreuveParcours, idAssuranceAnnulation, idInternauteInscriptionref, idInternauteReferent, montant, relais) ";			$bdd_assu .= "VALUE (".$id_epreuve.", ".$row['idParcours'].", ".$id_assurance_annulation.", ".$row['idInscriptionEpreuveInternaute'].",".$id_referent.", ".$search_assure['montant'].",'".$relais."')";  			$result_bdd_assu = $mysqli->query($bdd_assu);									$q1  = "UPDATE r_inscriptionepreuveinternaute SET assurance = ".$search_assure['montant']." ";			$q1 .= "WHERE idInscriptionEpreuveInternaute = ".$row['idInscriptionEpreuveInternaute'];			$mysqli->query($q1);						$q1  = "UPDATE r_insc_internaute_temp SET assurance = ".$search_assure['montant']." ";			$q1 .= "WHERE idInscriptionEpreuveInternaute = ".$row['idInscriptionEpreuveInternaute'];			$mysqli->query($q1);			$frais_assurance +=$search_assure['montant'];			
		}
			//$cout = $row['tarif']+$cout_option_indiv_2014;
				...
				{ 
					...
					$q1  = "UPDATE r_inscriptionepreuveinternaute SET frais_cb = ".$row['frais_cb']." ";
					$q1 .= "WHERE idInscriptionEpreuveInternaute = ".$row['idInscriptionEpreuveInternaute'];
					$mysqli->query($q1);
					//Enregistrement des frais supplémentaires quel que soit le payeur
					/*
		...
		/*** LOGS ****/
			...
			//echo $queryupriet;
			$resultupriet = $mysqli->query($queryupriet);
		/*** LOGS ****/		//***** fputs($fp," : CB / UPDATE r_insc_internaute_temp) : ".$queryupriet."\n");		/*** LOGS ****/
			$frais_cb_total += $row['frais_cb'];
		}
	...
	if ($type_paiement== 'CB') {
			...
			$tarif = $row_tarif['tarif'];*/
				//****** A VOIR if($payeur == 'coureur') $total = $cout+$frais_cb; else $total = $cout;
				//$total = $cout+$frais_assurance;
				$total = $cout;
			//pour test
			//$total=1;
				if ($nb_inscrit >1 && $id_referent!=0 ) {
					...
					$q1 .= "WHERE idInscriptionEpreuveInternaute=".$id_internauteInscription_ref;
					//echo $q1;
					//***$mysqli->query($q1);
					
					$q1  = "UPDATE r_insc_internautereferent SET frais_cb = ".round($frais_cb, 2).", ";
					$q1 .= "montant = ".round($total, 2)." ";
		...
		/*** LOGS ****/
					...
					$query_ref= "SELECT ri.nomInternaute, ri.prenomInternaute, ri.emailInternaute,rif.idInternauteref FROM r_internaute as ri ";
...
/*					
					...
					$bpaiements .= "VALUE ('M-".$id_referent."', ".round($total, 2).", ".round($frais_cb_total, 2).", NOW(), ".$row_ref['idInternauteref'].", ".$id_epreuve.", 'ATTENTE')";  
					$result = $mysqli->query($bpaiements);*/					$bpaiements  = "INSERT INTO b_paiements (reference, montant, frais_cb, date, id_referant, id_epreuve, statut, nomInternaute, prenomInternaute, emailInternaute) ";					$bpaiements .= "VALUE ('M-".$id_referent."', ".round($total, 2).", ".round($frais_cb_total, 2).", NOW(), ".$row_ref['idInternauteref'].", ".$id_epreuve.", 'ATTENTE','".addslashes_form_to_sql($row_ref['nomInternaute'])."','".addslashes_form_to_sql($row_ref['prenomInternaute'])."','".$row_ref['emailInternaute']."')";  					$result = $mysqli->query($bpaiements);
					$html_ca = create_form_ca("M-".$id_referent, round($total, 2), $row_ref['emailInternaute'],$id_epreuve,$iframe);
		...
		/*** LOGS ****/
				...
				else // pas de référent, inscription seule
				{
					...
					$q1  = "UPDATE r_inscriptionepreuveinternaute SET frais_cb = ".round($frais_cb_total, 2)." ";
					$q1 .= "WHERE idInscriptionEpreuveInternaute=".$id_referant_insc_seul;
					//echo $q1;
					//***$mysqli->query($q1);
/*
					$bpaiements  = "INSERT INTO b_paiements (reference, montant, frais_cb, date, id_referant, id_epreuve, statut) ";
					$bpaiements .= "VALUE ('".$id_referant_insc_seul."', ".round($total, 2).", ".round($frais_cb_total, 2).", NOW(), ".$id_internaute_referant_insc_seul.", ".$id_epreuve.", 'ATTENTE')";
					$result = $mysqli->query($bpaiements);
*/					$bpaiements  = "INSERT INTO b_paiements (reference, montant, frais_cb, date, id_referant, id_epreuve, statut, nomInternaute, prenomInternaute, emailInternaute) ";					$bpaiements .= "VALUE ('".$id_referant_insc_seul."', ".round($total, 2).", ".round($frais_cb_total, 2).", NOW(), ".$id_internaute_referant_insc_seul.", ".$id_epreuve.", 'ATTENTE','".addslashes_form_to_sql($nom_internaute_referant_insc_seul)."','".addslashes_form_to_sql($prenom_internaute_referant_insc_seul)."','".$email_referant_seul."')";					$result = $mysqli->query($bpaiements);				
					$html_ca = create_form_ca($id_referant_insc_seul, round($total, 2), $email_referant_seul,$id_epreuve,$iframe);
