...
global $mysqli;
		...
		*/
			...
			if ($id==0) $query .= " loginInternaute='".addslashes($login)."' AND passInternaute='".hhp($_POST['pass'])."' "; else $query .= " idInternaute='".$id."' ";
			//echo $query;
			$result = $mysqli->query($query);
...
function certif_medical_existe_light ($id_internaute,$query_update_certificat_fichier,$type_retour='fichier',$verif='non',$idInscriptionEpreuveInternaute=0)
	...
	//if ($idInscriptionEpreuveInternaute == 668769) echo $query;
	$result = $mysqli->query($query);
	$row= mysqli_fetch_row($result);
	//echo $row[0];
	if ($type_retour =='fichier')
...
function calcul_categorie($annee_naissance,$champ,$idTypeEpreuve=3,$sexe='MF',$idEpreuve='') {
	...
	$query .= " AND sexe = '".$sexe."'";
	if(!empty($nb)) $query .= " AND idEpreuve = ".$idEpreuve.""; else $query .= " AND idEpreuve IS NULL "; 
	$result = $mysqli->query($query);
	$row=mysqli_fetch_array($result);
	//echo $query;
	return $row;
...
global $mysqli;
	...
	if ($dossardmin > 0 && $dossardmax > 0 ) {
		...
		//print_r($dossard_affecte);
		//$nb_dossard_disponible = ($nb_dossard_du_parcours - $nb_exclusion-$nb_dossard_deja_attribue);
		//$nb_dossard_deja_attribue = 240;
		//echo "nb_dossard_du_parcours : ".$nb_dossard_du_parcours." - nb_exclusion : ".$nb_exclusion;
		$nb_de_dossard_dispo_total = $nb_dossard_du_parcours - $nb_exclusion;
		//$nb_dossard_or_exlus = 241;
		//if ($nb_dossard_or_exlus>0) $nb_dossard_disponible = ($nb_de_dossard_dispo_total - $nb_dossard_or_exlus); else $nb_dossard_disponible = ($nb_de_dossard_dispo_total - $nb_dossard_deja_attribue);
		$nb_dossard_disponible = ($nb_de_dossard_dispo_total - $nb_dossard_deja_attribue);
		//echo "nb_dossard_du_parcours : ".$nb_dossard_du_parcours." - nb_dossard_deja_attribue: ". $nb_dossard_deja_attribue." - nb_exclusion: ". $nb_exclusion." - nb_dossard_dans_exlus: ". $nb_dossard_dans_exlus;
		//$nb_dossard_disponible = ($nb_dossard_du_parcours - $nb_dossard_deja_attribue - ($nb_exclusion+$nb_dossard_dans_exlus));
	...
	$nb_info_dossard = array('nb_dossard_attribue'=>$nb_dossard_deja_attribue, 'nb_dossard_disponible'=>$nb_dossard_disponible, 'nb_dossard_reserve'=>$nb_exclusion, 'nb_dossard_parcours'=>$nb_dossard_du_parcours);
	//$nb_info_dossard = array('nb_de_dossard_dispo_total'=>$nb_de_dossard_dispo_total, 'nb_dossard_dans_exlus'=>$nb_dossard_dans_exlus, 'nb_dossard_or_exlus'=>$nb_dossard_or_exlus, 'nb_dossard_attribue'=>$nb_dossard_deja_attribue, 'nb_dossard_disponible'=>$nb_dossard_disponible, 'nb_dossard_reserve'=>$nb_exclusion, 'nb_dossard_parcours'=>$nb_dossard_du_parcours);
	return $nb_info_dossard;
...
global $mysqli;
	...
	if ($dossardmin > 0 && $dossardmax > 0 ) {
		...
		$nb_de_dossard_dispo_total = $nb_dossard_du_parcours - $nb_exclusion;
		/*
		echo 'nb_exclusion : '.$nb_exclusion;
		echo 'nb_plage_exclusion : '.$nb_plage_exclusion;
		echo 'nb_dossard_or_exlus : '.$nb_dossard_or_exlus;
		echo 'nb_dossard_dans_exlus : '.$nb_dossard_dans_exlus;
		*/
		if ($nb_dossard_or_exlus>0) $nb_dossard_disponible = ($nb_de_dossard_dispo_total - $nb_dossard_or_exlus); else $nb_dossard_disponible = ($nb_de_dossard_dispo_total - $nb_dossard_deja_attribue);
		$nb_dossard_disponible = $nb_dossard_du_parcours - $nb_dossard_deja_attribue - ($nb_exclusion-$nb_dossard_dans_exlus);
		//echo "nb_dossard_du_parcours : ".$nb_dossard_du_parcours." - nb_exclusion : ".$nb_exclusion;
		//echo "nb_de_dossard_dispo_total : ".$nb_de_dossard_dispo_total." - nb_dossard_deja_attribue : ".$nb_dossard_deja_attribue;
		//echo "nb_dossard_du_parcours : ".$nb_dossard_du_parcours." - nb_dossard_deja_attribue: ". $nb_dossard_deja_attribue." - nb_exclusion: ". $nb_exclusion." - nb_dossard_dans_exlus: ". $nb_dossard_dans_exlus;
		//$nb_dossard_disponible = ($nb_dossard_du_parcours - $nb_dossard_deja_attribue - ($nb_exclusion+$nb_dossard_dans_exlus));
...
//echo 'https://webservicesffa.athle.fr/st_chrono/stchrono.asmx/STCHRONO?UID=YASC&MDP=Atsats4!&NUMREL='.$num_lic.'&NOM='.$nom .'&PRENOM='.$prenom .'&SEXE='.$sexe.'&DATENAI='.$date_n.'&CMPCOD='.$CMPCOD.'&ID_ACT_EXT=&ID_CMP_EXT=&MSG_RETOUR=HTTP/1.1';
...
curl_setopt_array($curl, array(
    ...
    CURLOPT_URL => 'https://webservicesffa.athle.fr/st_chrono/stchrono.asmx/STCHRONO?UID=YASC&MDP=Atsats4!&NUMREL='.$num_lic.'&NOM='.$nom.'&PRENOM='.$prenom.'&SEXE='.$sexe.'&DATENAI='.$date_n.'&CMPCOD='.$CMPCOD.'&ID_ACT_EXT=&ID_CMP_EXT=&MSG_RETOUR=HTTP/1.1'
...
function evenements_a_venir( $nb = 5 )
{
	global $mysqli;
	$evenement = array();
	//cherche les épreuves avec un nombre limite de $nb
	...
	while( $row = mysqli_fetch_assoc( $result ) )
	{
		...
		//on compte le nombre d'épreuve pour les mettre en live
