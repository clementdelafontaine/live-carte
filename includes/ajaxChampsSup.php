...
error_reporting(E_ERROR);
	...
	global $mysqli;
		...
		$nb_info_dossard = nombre_dossard_v3($_GET['id_parcours']);
		...
		{ 
			...
			if ($_GET['orga'] != 'oui') {
				...
				if ( strtotime($row_tarifs['dateDebutTarif']) < $date_et_heure_du_jour AND $date_et_heure_du_jour < strtotime($row_tarifs['dateFinTarif'])) { 
					...
					$tar = tarif_reduc_place($row_tarifs['idEpreuveParcoursTarif']);
					//print_r($tar);
					//$json_tarif_promo['nb_dossard_tarif_max'] =  $tar['nb_dossard'] ;
					$json_tarif_promo['nb_dossard_tarif_pris'] =  $tar['nb_restant'] ;
					$json_tarif_promo['nb_dossard_reduction'] =  $tar['reduction'] ;
