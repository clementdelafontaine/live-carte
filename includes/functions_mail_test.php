...
if (!empty($donnees[0]['referents']) && $donnees[0]['insc_orga'] !=1) {
		...
		$query  = "SELECT DISTINCT (rii.idInscriptionEpreuveInternaute), rii.cout, rii.participation, rii.frais_cb, rii.frais_cheque, ri.prenomInternaute, ri.nomInternaute ";
		...
		echo $query .= " AND rii.idEpreuve = ".$donnees[0]['idEpreuve'];
		$result = $mysqli->query($query);
...
function send_mail_resultats ($expediteur_email ,$destinataire_email, $sujet, $corps, $pieces_jointes)
{
	...
	foreach( $pieces_jointes as $pj )
		$mail->addAttachment('/var/www/vhosts/ats-sport.com/httpdocs/tmp/'.$pj);
