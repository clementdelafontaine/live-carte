...
function questiondivers_file_existe ($id_fichier) {
...
function recup_champ_dotation_inscrit ($idEpreuve, $idEpreuveParcours, $idInternaute, $idInscriptionEpreuveInternaute) {
	...
	//$query .= "INNER JOIN r_inscriptionepreuveinternaute as riei ON ricd.idInternaute = riei.idInternaute ";
...
function calcul_frais_cb($tarif,$cout) {

	if($tarif == 0)
	{
		$frais_cb = 0;
		if($cout > 0) $frais_cb = $cout*0.025;
	}
	else if($tarif <= 25)
	{
		$frais_cb = 1.0;
		if($cout >=25) $frais_cb += ($cout - 25)*0.025;
	}
	else if($tarif <= 35)
	{
		$frais_cb = 1.5;
		if($cout >=35 ) $frais_cb += ($cout - 35)*0.025;
	}
	else if($tarif <= 50)
	{
		$frais_cb = 2.0;
		if($cout >=50) $frais_cb += ($cout - 50)*0.025;
	}
	else if($tarif > 50)
	{
		$frais_cb = 2.5;
...
function info_internaute_send_mail ($id_internaute,$cout,$type_paiement) {  
	...
	$query_info   .= "riit.idEpreuveParcoursTarif,riit.nomInternaute,riit.prenomInternaute,riit.emailInternaute,riit.dateInscription,riit.certificatMedical,riit.idSession,riit.codeSecurite, riit.autoParentale, riit.cout,riit.participation,riit.frais_cb,riit.frais_cheque,riit.reference_cheque,riit.idEpreuveParcoursTarifPromo, riit.code_promo,riit.valeur_code_promo,riit.info_diverses,riit.equipe,riit.groupe,  ";
...
function info_internaute_send_mail_test ($id_internaute,$cout,$type_paiement) {
	...
	$query_info   .= "rii.idEpreuveParcoursTarif,ri.nomInternaute,ri.prenomInternaute,ri.emailInternaute,rii.date_insc,ri.passInternaute, rii.verif_certif as certificatMedical, rii.verif_auto_parentale as autoParentale, rii.id_session as idSession,rii.paiement_montant,rii.montant_inscription as cout,rii.frais_cb,rii.frais_cheque,rii.info_cheque,rii.idEpreuveParcoursTarifPromo, rii.label_code_promo as code_promo,rii.montant_code_promo as valeur_code_promo,rii.info_diverses,rii.equipe, rii.groupe,   ";
